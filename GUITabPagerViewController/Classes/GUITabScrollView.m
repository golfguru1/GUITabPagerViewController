...
- (instancetype)initWithFrame:(CGRect)frame tabViews:(NSArray *)tabViews tabBarHeight:(CGFloat)height tabColor:(UIColor *)color backgroundColor:(UIColor *)backgroundColor {
    ...
    if (self) {
        ...
        [self setContentSize:CGSizeMake(MAX(width, self.frame.size.width) + frame.size.width, height)];
        ...
        for (UIView *tab in tabViews) {
            ...
            [VFL appendFormat:@"-(%f)-[T%d(%f)]", index ? 10.0f : frame.size.width / 2 - tab.frame.size.width/2, index, tab.frame.size.width];
            [views setObject:tab forKey:[NSString stringWithFormat:@"T%d", index]];
            
            [contentView addConstraints:[NSLayoutConstraint constraintsWithVisualFormat:@"V:|-20-[T]-2-|"
                                                                                options:0
                                                                                metrics:nil
                                                                                  views:@{@"T": tab}]];
            [tab setTag:index];
        ...
        [VFL appendString:[NSString stringWithFormat:@"-%f-|", 10.0f + frame.size.width / 2]];
...
- (void)_initTabbatAtIndex:(NSInteger)index {
    CGFloat x = self.frame.size.width / 2 - [self.tabViews[0] frame].size.width/2-5;
